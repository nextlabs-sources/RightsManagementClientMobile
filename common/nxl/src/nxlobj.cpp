...
void nxl::FmtHeader::validate() {
    ...
    // Check Algorithm
    if (NXL_ALGORITHM_AES128 != header_.Basic.Algorithm && NXL_ALGORITHM_AES256 != header_.Basic.Algorithm) {
...
void nxl::FmtHeader::create(const unsigned char* owner_id, const NXL_CRYPTO_TOKEN* crypto_token, const void* recovery_token) {
    // sanity check
    ...
    aes_encrypt_cbc(kek, 32, (char*) cek, 32, header_.Crypto.IVSeed, sizeof(header_.Crypto.IVSeed));
    memcpy(header_.Crypto.TPCEK, cek, 32);
    
    // do hmac_sha256
    char hashcek[32] = {0};
    hmac_sha256_token((char*)crypto_token->Token, sizeof(crypto_token->Token), (char*)cekey_, sizeof(cekey_), hashcek);
    ...
    memcpy(header_.Crypto.PublicKey2, crypto_token->PublicKeyWithiCA, sizeof(crypto_token->PublicKeyWithiCA));
    ...
    hmac_sha256_token((char*)crypto_token->Token, sizeof(crypto_token->Token), nullptr, 0, hashEmptySection);
    ...
    // .Policy
    memcpy(header_.SectionHeader.records[1].Name, BUILDINSECTIONPOLICY, strlen(BUILDINSECTIONPOLICY));
...
void nxl::FmtHeader::update_header_hash(const NXL_CRYPTO_TOKEN* cryptotoken)
{
    ...
    int len = (char*)&(header_.DynamicHeader) - pstart;
    hmac_sha256_token((char*)cryptotoken->Token, sizeof(cryptotoken->Token), pstart, len, hashheader);
...
int nxl::FmtHeader::get_scn_index(const char* name) const throw () {
    for (int i = 0; i < (int) get_scns_count(); ++i) {
        if (icasecmp(std::string(name), std::string((char*)header_.SectionHeader.records[i].Name))) {
...
void nxl::FmtHeader::update_section(int index, const char *data, uint16_t datalen, uint16_t original_len, uint16_t compressed_len, uint32_t flag, const NXL_CRYPTO_TOKEN *cryptotoken){
    if (index < 0 || index >= (sizeof(header_.SectionHeader.records) / sizeof(NXL_SECTION_RECORD))) {
        return;
    }
...
void nxl::FileStream::prepare_for_encrypt(const unsigned char* owner_id, const NXL_CRYPTO_TOKEN* crypto_token, const void* recovery_token) {
    if (!is_open()) {
...
void nxl::FileStream::read_ownerid(const char *name, void *pb, uint32_t *pcb, bool validate, const NXL_CRYPTO_TOKEN* cryptotoken)
{
    if (!name || !pb || !pcb) {
    ...
    *pcb = header_.get_bsc_ownerid_size();
    memcpy(pb, header_.get_bsc_ownerid(), *pcb);
}


void nxl::FileStream::read_section(const char* name, void* pb, uint32_t* pcb, uint32_t* flag, bool validate, const NXL_CRYPTO_TOKEN* cryptotoken) {
    
    header_.validate_nxl_header(cryptotoken);  // if nxl file was changed unexpectly, then just throw exception and stop
...
void nxl::FileStream::write_section(const char* name, const void* pb, uint32_t cb, uint32_t flag, const NXL_CRYPTO_TOKEN* cryptotoken) {
    header_.validate_nxl_header(cryptotoken);
...
void nxl::FileStream::aes_cbc_operator(bool encrypt, char* buf, int buflen, uint64_t offset) {
    ...
    for (int i = 0; i < blocks; i++) {
        ...
        if (encrypt) {
            aes_encrypt_cbc((char*) header_.get_cekey(), 32, buf, NXL_CBC_SIZE, ivec, sizeof(ivec));
        } else {
            aes_decrypt_cbc((char*) header_.get_cekey(), 32, buf, NXL_CBC_SIZE, ivec, sizeof(ivec));
        }
