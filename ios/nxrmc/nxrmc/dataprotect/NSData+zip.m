...
- (NSData *)gzip {
    ...
    zlibStreamStruct.avail_in  = [NSNumber numberWithUnsignedInteger:[pUncompressedData length]].unsignedIntValue; // Number of input bytes left to process
    ...
    do {
        ...
        zlibStreamStruct.avail_out = [NSNumber numberWithUnsignedInteger:[compressedData length]].unsignedIntValue - [NSNumber numberWithUnsignedLong:zlibStreamStruct.total_out].unsignedIntValue;
...
- (NSData *)ungzip {
    ...
    NSData *pCompressedData = self;
    if ([pCompressedData length] == 0) {
        return pCompressedData;
    }
    
    unsigned long full_length = [pCompressedData length];
    unsigned long half_length = [pCompressedData length] / 2;
    ...
    strm.avail_in =[NSNumber numberWithUnsignedInteger:[pCompressedData length]].unsignedIntValue;
    strm.total_out = 0;
    strm.zalloc = Z_NULL;
    strm.zfree = Z_NULL;
    ...
    while (!done) {
        ...
        strm.avail_out = [NSNumber numberWithUnsignedInteger:[decompressed length]].unsignedIntValue - [NSNumber numberWithUnsignedLong:strm.total_out].unsignedIntValue;
